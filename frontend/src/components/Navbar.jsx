...
const Navbar = () => {
  ...
  const getDashboardBase = () => {
    ...
    if (user.role === 'site') return '/site/dashboard';
    return '/';
  };

  const getAuthedLinks = () => {
    if (!user) return [];
    if (user.role === 'admin') {
      return [
        { to: getDashboardBase(), label: 'Overview' },
        { to: `${getDashboardBase()}?tab=users`, label: 'Users' },
        { to: `${getDashboardBase()}?tab=sites`, label: 'Sites' },
        { to: `${getDashboardBase()}?tab=studies`, label: 'Studies' },
        { to: `${getDashboardBase()}?tab=leads`, label: 'Leads' },
      ];
    }
    if (user.role === 'sponsor') {
      return [
        ...
        { to: `${getDashboardBase()}?tab=studies`, label: 'Studies' },
        { to: `${getDashboardBase()}?tab=sites`, label: 'Sites' },
        { to: `${getDashboardBase()}?tab=participants`, label: 'Participants' },
      ];
    }
    // site
    return [
      { to: getDashboardBase(), label: 'Overview' },
      { to: `${getDashboardBase()}?tab=participants`, label: 'Participants' },
      { to: `${getDashboardBase()}?tab=workflow`, label: 'Workflow' },
    ];
  };
        ...
        <div className="flex items-center justify-between h-24">
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-center space-x-2" role="menubar">
              ...
              {user && (
                <>
                  {getAuthedLinks().map(link => (
                    <Link
                      key={link.to}
                      to={link.to}
                      onClick={() => handleNavClick(link.to)}
                      className={`${getButtonStyles(link.to.includes('?') ? link.to.split('?')[0] : link.to)} focus:outline-none focus:ring-2 focus:ring-[#56F0C8] focus:ring-offset-2`}
                      role="menuitem"
                    >
                      {link.label}
                    </Link>
                  ...
                  <div className="ml-4 relative">
                    <button
                      ...
                      className="flex items-center space-x-2 px-3 py-2 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#56F0C8] focus:ring-offset-2"
                      aria-haspopup="true"
                      aria-expanded={isProfileOpen}
                    >
                      <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-[#16B1F0] text-white font-bold">
                        {user.name?.charAt(0)?.toUpperCase() || 'U'}
                      </span>
                      <span className="text-sm font-medium text-[#10224E]">{user.name || 'User'}</span>
              ...
              <>
                {getAuthedLinks().map(link => (
                  <Link
                    key={link.to}
                    to={link.to}
                    className={`block px-3 py-2 rounded-md text-base font-medium ${isActive(link.to.includes('?') ? link.to.split('?')[0] : link.to) ? 'bg-[#56F0C8] text-[#0B1220]' : 'text-slate-600 hover:text-slate-900'} focus:outline-none focus:ring-2 focus:ring-[#56F0C8] focus:ring-offset-2`}
                    onClick={() => {
                      setIsMobileMenuOpen(false);
                ...
                <Link
                  to="/settings"
                  className="block px-3 py-2 rounded-md text-base font-medium text-slate-600 hover:text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#56F0C8] focus:ring-offset-2"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  Settings
                </Link>
